...
// --- Perbaikan data SEKALI PAKAI (2026-08-11) ---
// Bug lama di customer.js: customer LAMA yang race lewat form "Masuk"
// (ngetik nama & klik sebelum auto-rejoin diam-diam selesai di background)
// bikin lastMessage/lastSender ke-reset kosong/null dan lastMessageAt
// ke-refresh ke waktu form itu diklik, PADAHAL riwayat pesan aslinya
// (chats/{uid}/messages) sama sekali gak berubah -- sidebar jadi nunjukin
// baris "kosong" dengan jam yang salah. Sudah diperbaiki di customer.js
// (gak akan kejadian lagi ke depannya); fungsi ini cuma buat MEMPERBAIKI
// dokumen yang SUDAH kena sebelum perbaikan itu naik.
// Cara pakai: buka dashboard admin, login, buka DevTools Console (F12),
// ketik repairCustomerPreviews() lalu Enter. Aman dijalankan berkali-kali
// (idempotent -- customer yang preview-nya udah benar/emang belum pernah
// chat dilewati begitu saja). HAPUS fungsi ini di commit berikutnya setelah
// dipakai, ini bukan fitur permanen.
window.repairCustomerPreviews = async function () {
  if (!currentAdmin) {
    console.error("Login dulu sebagai admin sebelum menjalankan ini.");
    return;
  }
  const candidatesSnap = await getDocs(query(collection(db, ...wsPath("customers")), where("lastMessage", "==", "")));
  console.log(`Ditemukan ${candidatesSnap.size} customer dengan preview kosong, mengecek riwayat aslinya...`);

  let fixed = 0;
  for (const customerDoc of candidatesSnap.docs) {
    const uid = customerDoc.id;
    const lastMsgSnap = await getDocs(
      query(collection(db, ...wsPath("chats", uid, "messages")), orderBy("timestamp", "desc"), limit(1))
    );
    if (lastMsgSnap.empty) continue; // beneran belum pernah chat, bukan kena bug -- biarkan

    const msg = lastMsgSnap.docs[0].data();
    const preview = msg.type === "image" ? "📷 Gambar" : msg.type === "options" ? "📋 Menu pilihan" : msg.text || "";
    await setDoc(
      doc(db, ...wsPath("customers", uid)),
      { lastMessage: preview, lastMessageAt: msg.timestamp || serverTimestamp(), lastSender: msg.sender || null },
      { merge: true }
    );
    fixed++;
    console.log("Diperbaiki:", customerDoc.data().name || uid, "->", JSON.stringify(preview));
  }
  console.log(`Selesai. ${fixed} customer diperbaiki dari ${candidatesSnap.size} yang dicek.`);
};
